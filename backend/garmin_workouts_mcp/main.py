...
def generate_workout_data_prompt(description: str) -> dict:
    ...
    # Example user profile (paces can be personalized later)
    USER_PACES = {
        "HM": "3:35 min/km",
        "5k": "3:20 min/km",
        "10k": "3:25 min/km"
    ...
    User Profile Paces:
    - HM pace = {USER_PACES['HM']}
    - 5k pace = {USER_PACES['5k']}
    - 10k pace = {USER_PACES['10k']}

    Rules:
    1. Only include "heart rate" targets if the workout step explicitly specifies HR zones (Z1–Z5).
       - If zone is Z0 or no HR zone is given, set target.type = "no target".
    2. Do not use pace targets. If text mentions HM/5k/10k pace, write it as a short note in "stepDescription".
    3. Use stepDescription notes that are short and minimal (a couple of words).
    4. Keep all steps structured with correct endConditionType ("time" or "distance").
    ...
    "steps": [
        {{
        "stepName": "Step Name",
        "stepDescription": "short note",
        "endConditionType": "time" | "distance",
        "stepDuration": seconds_if_time,
        "stepDistance": value_if_distance,
        "distanceUnit": "m" | "km",
        "stepType": "warmup" | "interval" | "recovery" | "repeat",
        "target": {{
            "type": "no target" | "heart rate",
            "value": [min, max] or null,
            "unit": "bpm" or null
        }},
